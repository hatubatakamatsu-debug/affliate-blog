...
def build_prompt(genre: dict, persona: dict) -> str:
    ...
    return f"""あなたは{genre['label']}専門のアフィリエイトブログライターです。
以下の条件で、SEOに強くかつ読者の役に立つブログ記事を日本語で書いてください。
...
2. リード文（200文字、読者の悩みに共感し、記事を読む理由を明示）
3. H2見出し3〜4個（各500文字程度）
4. まとめ（200文字、行動を促すCTA含む）
...
- 読者の立場（{persona['key']}）に合った言葉遣い・悩みの描写にする
- 「〜です。〜ます。」調の丁寧語で統一する
"""
