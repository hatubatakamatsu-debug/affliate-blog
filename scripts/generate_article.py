...
- ユーモアは「ちょっと共感できる」程度では弱すぎます。運営者自身の具体的なミニエピソード（小さな失敗・墓穴・矛盾した行動など）を、オチがつく形で1〜2箇所は入れてください。例えば「〜しようとしていたのに、実際には（現実的な理由で）〜という残念な方向転換をした」というような、読者が思わずクスッと笑える"あるあるオチ"です。ただし悩みの深刻さを軽視するようなブラックユーモアや皮肉にはしない
